...
<template>
  <div class="music-list-view space-y-6">
    <div class="flex justify-between items-center border-b-4 border-black pb-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-[10px] font-bold text-ts-blue uppercase tracking-widest mt-1">
            Status: Conectado ao Localhost
          </p>
        </div>
      </div>
      ...
      <Dialog v-model:open="isOpen">
        ...
        <DialogContent
          class="fixed left-[50%] top-[50%] z-50 grid w-full max-w-2xl translate-x-[-50%] translate-y-[-50%] bg-ts-retro-gray border-4 border-black rounded-none shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] p-0 gap-0 overflow-hidden outline-none max-h-[90vh] flex flex-col"
        >
          <DialogHeader
            class="bg-ts-retro-blue p-2 border-b-2 border-black flex-shrink-0"
          >
            <DialogTitle
              class="text-white text-[10px] uppercase font-black tracking-widest flex items-center gap-2"
            >
              <div
                class="w-3 h-3 bg-white border border-black shadow-[1px_1px_0px_rgba(0,0,0,0.5)]"
              ></div>
    ...
    >
      ...
      <span class="font-black animate-pulse uppercase text-xs tracking-tighter"
        >Acessando_Disco_Rigido...</span
      >
    </div>
    ...
    <div
      v-else
      class="border-4 border-dashed border-black/20 p-20 text-center bg-gray-50"
    >
      ...
      <Button
        variant="link"
        @click="isOpen = true"
        class="mt-2 text-ts-blue font-bold uppercase text-xs"
      >
