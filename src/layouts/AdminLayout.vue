...
<template>
  <div class="min-h-screen flex font-mono text-ts-black overflow-hidden">
    ...
    >
      <div
        class="p-4 border-b-4 border-black text-black flex items-center justify-between overflow-hidden"
      >
        ...
        >
          ...
          <Button
            ...
            :class="[
              ...
              isActive(item.path)
                ? 'bg-white shadow-[4px_4px_0px_0px_black] translate-x-[-2px] translate-y-[-2px]'
          ...
          >
            <component
              :is="item.icon"
              :size="20"
              :class="isActive(item.path) ? 'text-ts-pink' : ''"
            />
            <span v-if="!isCollapsed" class="font-black uppercase text-xs">{{
              item.name
            }}</span>
          </Button>
        </div>
      ...
      <div v-if="!isCollapsed" class="p-4 space-y-4 border-t-4 border-black bg-gray-300">
        <div class="flex justify-center gap-2 overflow-hidden">
          <img
            src="https://blob.gifcities.org/gifcities/2ZL6GSLIX76ROTHWO5FJPLXGA6VYHAIO.gif"
            alt="badges"
            class="pixelated h-8"
          />
          <img
            src="https://cyber.dabamos.de/88x31/button.gif"
            alt="badge"
            class="pixelated h-8"
          />
        </div>
        ...
        <div
          class="flex items-center justify-center gap-2 grayscale hover:grayscale-0 transition-all"
        >
          <img
            src="https://blob.gifcities.org/gifcities/JFFM5775ELYWARHSCWKVECD4PZ6IFSBB.gif"
            class="h-3 pixelated"
          />
          <span class="text-[8px] font-black uppercase text-gray-500 tracking-tighter"
            >Estrela_OS v4.2.0</span
          >
        </div>
      </div>
    </aside>
